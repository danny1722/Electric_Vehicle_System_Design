...
from old_code.battery_simulation import optimize_storage
acc = 0.5       # m/s²
dec = 0.5       # m/s²
mass = 83000    # kg
...
def main():
    ...
    total_station_stops = np.concatenate((station_stops, reverse_station_stops + route_length))
    print(f"Total station stops: {total_station_stops}")
    ...
    total_speed_limit_dist = np.concatenate((speed_limit_dist, reverse_speed_limit_dist + route_length))
    print(f"Total speed limit distances: {total_speed_limit_dist}")
    ...
    print(f"Electrified stations: {total_electrified_stations}")
